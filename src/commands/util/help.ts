	...
	public async exec(msg: Message, { command }: { command: Command | null }): Promise<Message | Message[]> {
		...
		if (!command) {
			const embed = this.client.util.embed()
				...
				.setFooter('Made with Love by litterbøx ✿#0001 💖')
				.setTitle('📃 Commands')
			...
			for (const category of this.handler.categories.values()) {
				...
				embed.addField(`\`💼\` ${category.id}`, `${category.filter(cmd => cmd.aliases.length > 0).map(cmd => `\`${cmd.aliases[0]}\`${cmd.description && cmd.description.content ? ` - ${cmd.description.content.split('\n')[0].substring(0, 120)}` : ''}`).join('\n') || 'Nothin\' to see here! '}`);
			}
		...
		const embed = this.client.util.embed()
			.setColor(this.client.config.color!)
			.setFooter('Made with Love by litterbøx ✿#0001 💖')
			.setTitle(`\`${this.client.config.prefix}${command.aliases[0]} ${command.description.usage ? command.description.usage : ''}\``)
		...
		if (command.description.examples && command.description.examples.length) embed.addField('» Examples', `\`${command.aliases[0]} ${command.description.examples.map((e: string): string => `${this.client.config.prefix}${e}`).join('\n')}\``);
		if (command.userPermissions) {
			const permissions = command.userPermissions as Array<PermissionResolvable>;
			embed.addField('» Required Permissions', permissions.map((i: PermissionResolvable): string => {
